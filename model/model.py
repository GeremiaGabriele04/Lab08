from database.DAO import DAO


class Model:
    def __init__(self):
        self._solBest = []
        self._bestValue = 0
        self._listNerc = None
        self._listEvents = None
        self.loadNerc()



    def worstCase(self, nerc, maxY, maxH):
        self.loadEvents(nerc)

        # Ordino per data (utile per vincolo anni)
        self._listEvents.sort(key=lambda x: x.date_event_began)

        self._solBest = []
        self._bestValue = 0

        self.ricorsione([], maxY, maxH, 0)

        return [self._solBest, self._bestValue]

    def ricorsione(self, parziale, maxY, maxH, pos):

        # calcolo ore totali
        ore_tot = sum(
            (e.date_event_finished - e.date_event_began).total_seconds() / 3600
            for e in parziale
        )

        if ore_tot > maxH:
            return

        # vincolo anni
        if len(parziale) > 0:
            anni = [e.date_event_began.year for e in parziale]
            if max(anni) - min(anni) > maxY:
                return

        # calcolo persone
        persone = sum(e.customers_affected for e in parziale)

        # aggiorno soluzione migliore
        if persone > self._bestValue:
            self._bestValue = persone
            self._solBest = parziale.copy()

        # espansione
        for i in range(pos, len(self._listEvents)):
            e = self._listEvents[i]

            parziale.append(e)
            self.ricorsione(parziale, maxY, maxH, i + 1)
            parziale.pop()



    def loadEvents(self, nerc):
        self._listEvents = DAO.getAllEvents(nerc)

    def loadNerc(self):
        self._listNerc = DAO.getAllNerc()


    @property
    def listNerc(self):
        return self._listNerc